...
U (up), and D (down). If the robot returns to the origin after it finishes 
all of its moves, return true. Otherwise, return false.

Note: The way that the robot is "facing" is irrelevant. 
"R" will always make the robot move to the right once,
 "L" will always make it move left, etc. Also, assume that 
...
class Solution:
    ...
    def judgeCircle(self, moves: str) -> bool:
	    if moves.count('R') == moves.count('L') and moves.count('D') == moves.count('U'):
		    return True
	    else: 
		    return False
